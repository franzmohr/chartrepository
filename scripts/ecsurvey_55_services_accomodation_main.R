...
filter_specific_countries <- !is.null(ctry_code)
...
ecdata <- readxl::read_xlsx(temp_sdmx_files, sheet = "55", na = "NA", col_types = c("date", rep("numeric", 245))) %>%
  rename(date = `55`) %>%
...
if (lang == "de") {
  ...
  temp_title <- "Beherbergung (EK Umfrage)"
  temp_caption <- "Quelle: Europäische Kommission. Subsektor 93 (Accomodation). Saisonal bereinigte Daten."
}
...
if (lang == "en") {
  ...
  temp_title <- "Accomodation (EC survey)"
  temp_caption <- "Source: European Commission. Subsector 55 (Accomodation). Seasonally adjusted series."
}
...
ggsave(g, filename = "figures/ecsurvey_55_services_accomodation_main.png", height = 7, width = 7)
