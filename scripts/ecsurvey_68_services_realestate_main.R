...
ecdata <- readxl::read_xlsx(temp_sdmx_files, sheet = "68", na = "NA", col_types = c("date", rep("numeric", 245))) %>%
  rename(date = `68`) %>%
...
if (lang == "de") {
  ...
  temp_title <- "Immobiliendienstleistungen (EK Umfrage)"
  temp_caption <- "Quelle: Europäische Kommission. Subsektor 68 (Real Estate Activities). Saisonal bereinigte Daten."
}
...
if (lang == "en") {
  ...
  temp_title <- "Real estate activities (EC survey)"
  temp_caption <- "Source: European Commission. Subsector 68 (real estate activities). Seasonally adjusted series."
}
...
ggsave(g, filename = "figures/ecsurvey_68_services_realestate_main.png", height = 7, width = 7)
