...
ecdata <- readxl::read_xlsx(temp_sdmx_files, sheet = "64", na = "NA", col_types = c("date", rep("numeric", 245))) %>%
  rename(date = `64`) %>%
...
if (lang == "de") {
  ...
  temp_title <- "Finanzdienstleistungen ohne Versicherungen (EK Umfrage)"
  temp_caption <- "Quelle: Europäische Kommission. Subsektor 64 (Finanzdienstleistungen ohne Versicherungen). Saisonal bereinigte Daten."
}
...
if (lang == "en") {
  ...
  temp_title <- "Financial services without insurance (EC survey)"
  temp_caption <- "Source: European Commission. Subsector 64 (financial services witout insurance). Seasonally adjusted series."
}
...
ggsave(g, filename = "figures/ecsurvey_64_services_financial_main.png", height = 7, width = 7)
