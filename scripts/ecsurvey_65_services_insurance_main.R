# ecsurvey, finance, services, insurance
...
ecdata <- readxl::read_xlsx(temp_sdmx_files, sheet = "65", na = "NA", col_types = c("date", rep("numeric", 245))) %>%
  rename(date = `65`) %>%
...
if (lang == "de") {
  ...
  temp_title <- "Versicherungsdienstleistungen (EK Umfrage)"
  temp_caption <- "Quelle: Europäische Kommission. Subsektor 65 (Versicherungsdienstleistungen). Saisonal bereinigte Daten."
}
...
if (lang == "en") {
  ...
  temp_title <- "Insurance services (EC survey)"
  temp_caption <- "Source: European Commission. Subsector 65 (insurance services). Seasonally adjusted series."
}
...
ggsave(g, filename = "figures/ecsurvey_65_services_insurance_main.png", height = 7, width = 7)
