# macro, forecast, unemployment, projection
...
actual <- get_eurostat(id = "une_rt_a",
  ...
  rename(date = time,
         ctry = geo,
         value = values) %>%
  mutate(ctry = ifelse(ctry == "EA20", "EA", ctry)) %>%
...
ggsave(g, filename = "figures/unemployment_projections_for_eu_countries.png", height = 3.5, width = 8)
